...
export class PortletUtils {
  ...
  public createResourceURL(serviceIp: string, serverPort: string, p_l_id: string, p_p_id: string, pluginPortletName: string, resourceId: string):string {
    ...
    return `http://${serviceIp}:${serverPort}/c/portal/layout?p_p_cacheability=cacheLevelPage&p_p_lifecycle=2&p_l_id=${p_l_id}&p_p_id=${p_p_id}_WAR_${pluginPortletName}&p_p_resource_id=${resourceId}`;

  }

  public createDefaultResourceURL(p_p_id: string, resourceId: string):string {

    return `http://${this._serverIp}:${this._serverPort}/c/portal/layout?p_p_cacheability=cacheLevelPage&p_p_lifecycle=2&p_l_id=${this._p_l_id}&p_p_id=${p_p_id}_WAR_${this._pluginPortletName}&p_p_resource_id=${resourceId}`;
