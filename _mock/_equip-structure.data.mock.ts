...
function dataGenerator(req: MockRequest) {
  ...
  for(let i = 0; i < train_base_info.groups; i++){
    ...
    let id = `${sn}_coach_${(i+1).toString().padStart(2, '0')}`;

    if(randomTrueFalse()){
      result.areasHaveProblem.push(id)
    }

    result.areas.push({
      id: id,
      name: `${i+1}车`,
      image: 'assets/data/train-head.png'
    })
  }
...
function areaParamsDataGenerator(req: MockRequest) {
  ...
  for(let i= 0; i < hotSpotNum; i++){
    ...
    hotSpots.push({
      id: `hotspot_${i}`,
      coordinate: [
        LONGITUDE_RANGE[0] + Math.floor( (LONGITUDE_RANGE[1] - LONGITUDE_RANGE[0] ) * Math.random()),
        LATITUDE_RANGE[0] + Math.floor( (LATITUDE_RANGE[1] - LATITUDE_RANGE[0] ) * Math.random())],
        params: [
        generatedParam
      ]
    })
